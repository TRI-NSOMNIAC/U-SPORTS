...
router.patch('/:id', requireAuth, requireRole('Organizer', 'Admin'), async (req: AuthRequest, res) => {
  ...
  const schema = z
    ...
    .refine((o) => o.name !== undefined || o.description !== undefined || o.table_tennis_format !== undefined, {
      message: 'Provide name, description, and/or table_tennis_format',
    })
...
router.post('/', requireAuth, requireRole('Organizer', 'Admin'), async (req: AuthRequest, res) => {
  ...
  try {
    ...
    const { description: rawDescription, table_tennis_format, ...rest } = body
    ...
    const { data, error } = await supabase
      .from('events')
      .insert({ ...rest, description, table_tennis_format: ttFormat, created_by: req.user!.id })
      .select()
      .single()
