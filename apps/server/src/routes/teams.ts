...
router.post('/:id/members', requireAuth, requireRole('Organizer', 'Admin', 'Coach'), async (req: AuthRequest, res) => {
  ...
  const { data: athlete, error: athleteErr } = await supabase
    .from('athletes')
    .select('id, sport, verification_status, profile_id')
  ...
  if (athlete.verification_status !== 'approved') {
    return res.status(400).json({ error: 'Athlete profile must be roster-approved before competition placement' })
  }
