...
import { Play, Users, Trophy, Shuffle, Tv2, Calendar, UserCheck, Trash2, Ban, Pencil } from 'lucide-react'
import { Button, Card, Badge, Modal, Select, Alert, TabBar, Skeleton, Table, Input, Textarea } from '../../components/ui'
import api from '../../lib/api'
import { supabase } from '../../lib/supabase'
import type { Event, Match, Bracket, TryoutRegistration, Team } from '../../types'
...
export default function OrganizerEventDetail() {
  ...
  const openMatchWorkspace = (m: Match) => {
    ...
    else navigate(`/organizer/scoring/${m.id}`)
  }
  ...
  const [tryoutRegs, setTryoutRegs] = useState<TryoutRegistration[]>([])
  const [tryoutLoading, setTryoutLoading] = useState(false)
  const [updatingTryout, setUpdatingTryout] = useState<string | null>(null)
  ...
  const [tryoutDecisionConfirm, setTryoutDecisionConfirm] = useState<{
    regId: string
  ...
  const confirmTryoutDecision = async () => {
    if (!tryoutDecisionConfirm || !id) return
    setUpdatingTryout(tryoutDecisionConfirm.regId)
    setError('')
    try {
      await api.patch(`/events/${id}/tryout/registrations/${tryoutDecisionConfirm.regId}`, {
        status: tryoutDecisionConfirm.status,
      })
      setTryoutDecisionConfirm(null)
      const r = await api.get<TryoutRegistration[]>(`/events/${id}/tryout/registrations`)
      setTryoutRegs(r.data ?? [])
    } catch (e: unknown) {
      const msg =
        e && typeof e === 'object' && 'response' in e
          ? (e as { response?: { data?: { error?: string } } }).response?.data?.error
  ...
  useEffect(() => {
    if (!id || !event?.is_tryout) return
    setTryoutLoading(true)
    api
      .get<TryoutRegistration[]>(`/events/${id}/tryout/registrations`)
      .then((r) => setTryoutRegs(r.data ?? []))
  ...
  const refreshTryouts = () =>
    api.get<TryoutRegistration[]>(`/events/${id}/tryout/registrations`).then((r) => setTryoutRegs(r.data ?? []))
  ...
  const readOnlyEvent = !canConfigureSport(event.sport)

  const rosterMatchesEventKind = (t: Pick<Team, 'roster_context'> | { roster_context?: string | null }) => {
    const ctx = t.roster_context === 'tryout' ? 'tryout' : 'official'
    return event.is_tryout ? ctx === 'tryout' : ctx === 'official'
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {readOnlyEvent && (
        <Alert type="info">
          View-only for this sport: your organizer account cannot edit {getSportLabel(event.sport as any)} events. Browse bracket
          and matches or open scoring; use another organizer account with this sport assigned to make changes.
        </Alert>
      )}
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          {event.is_tryout && <Badge className="mt-2" variant="info">Tryout</Badge>}
        ...
        <div className="flex flex-col items-stretch sm:items-end gap-2 shrink-0">
          ...
          <div className="flex flex-wrap items-center gap-2 sm:justify-end">
            <Badge variant={STATUS_VARIANTS[event.status]}>{organizerEventStatusLabel(event.status, event.is_tryout)}</Badge>
            {event.status === 'draft' && event.is_tryout && (
              <Button size="sm" onClick={() => void handleStatusChange('registration')}>Open tryout registration</Button>
            )}
            {event.status === 'draft' && !event.is_tryout && (
              <Button
                size="sm"
              ...
              <div className="flex gap-2">
                {(['singles', 'doubles'] as const).map((fmt) => (
                  <button
                    ...
                    className={`flex-1 py-2 rounded-lg text-sm font-medium border transition-colors ${editTTFormat === fmt ? 'bg-[#0066FF] border-[#0066FF] text-white' : 'bg-[var(--surface-elevated)] border-[var(--border-subtle)] text-[var(--text-muted)] hover:text-white'}`}
                  >
      ...
      <TabBar
        tabs={[
          ...(event.is_tryout
            ? [{ id: 'tryouts', label: `Tryouts (${tryoutRegs.length})`, icon: <UserCheck className="w-3.5 h-3.5" /> }]
            : []),
      ...
      {!event.is_tryout && event.status === 'completed' && eliminationPodium && eliminationPodium.length > 0 ? (
        <EventPodiumStrip placements={eliminationPodium} labelByParticipantId={participantLabels} title="Final standings" />
      ) : null}

      {tab === 'tryouts' && event.is_tryout && (
        <Card className="p-4">
          <p className="text-sm text-[var(--text-muted)] mb-4">
            Mark <strong>Selected</strong> to promote students to athletes for this sport (they can upload medical next).{' '}
            <strong>Not selected</strong> keeps them as verified students for future tryouts.
          </p>
          {tryoutLoading ? (
            ...
            <Table
              ...
              data={tryoutRegs.map((tr) => ({
                name: (
                  <div>
                    <p className="font-medium">{(tr as TryoutRegistration & { profile?: { full_name?: string; email?: string } }).profile?.full_name ?? '—'}</p>
                    <p className="text-xs text-[var(--text-muted)]">{(tr as any).profile?.email}</p>
                  </div>
                ),
                status: <Badge variant={tr.status === 'selected' ? 'success' : tr.status === 'not_selected' ? 'default' : 'info'}>{formatEnumLabel(tr.status)}</Badge>,
                actions: (
                  <div className="flex flex-wrap gap-1">
                    <Button
                      size="sm"
                      variant="success"
                      loading={updatingTryout === tr.id}
                      disabled={readOnlyEvent || tr.status === 'selected'}
                      onClick={() =>
                        setTryoutDecisionConfirm({
                          regId: tr.id,
                          status: 'selected',
                          athleteName:
                            (tr as TryoutRegistration & { profile?: { full_name?: string } }).profile?.full_name ?? 'this student',
                        })
                      }
                    >
                      Selected
                    ...
                    <Button
                      ...
                      variant="secondary"
                      loading={updatingTryout === tr.id}
                      disabled={readOnlyEvent || tr.status === 'not_selected'}
                      onClick={() =>
                        setTryoutDecisionConfirm({
                          regId: tr.id,
                          status: 'not_selected',
              ...
              emptyMessage="No tryout registrations yet."
            />
          )}
        </Card>
      )}
          ...
          <Card>
            ...
            <div className="flex gap-3">
              <Select
                ...
                disabled={readOnlyEvent}
              />
              <Button loading={addingParticipant} onClick={handleAddParticipant} disabled={readOnlyEvent || !selectedTeam}>
                Add
              </Button>
            </div>
            {teamsWrongRosterKind ? (
              <p className="text-xs text-[var(--text-muted)] mt-3">
                {event.is_tryout
                  ? 'Only tryout squads for this season are listed. Official teams are hidden.'
                  : 'Only official competition rosters are listed. Tryout squads belong on tryout events.'}
              </p>
            ) : null}
          </Card>
      ...
      <Modal
        open={tryoutDecisionConfirm !== null}
        onClose={() => !updatingTryout && setTryoutDecisionConfirm(null)}
        title="Confirm tryout outcome"
        size="md"
      >
        {tryoutDecisionConfirm && (
          <div className="space-y-4">
            <Alert type={tryoutDecisionConfirm.status === 'selected' ? 'info' : 'warning'}>
              Mark <strong>{tryoutDecisionConfirm.athleteName}</strong> as{' '}
              <strong>{tryoutDecisionConfirm.status === 'selected' ? 'selected' : 'not selected'}</strong>? This updates their tryout registration for this event.
            </Alert>
            <div className="flex justify-end gap-2">
              <Button variant="secondary" onClick={() => setTryoutDecisionConfirm(null)} disabled={!!updatingTryout}>
                Cancel
              </Button>
              <Button loading={updatingTryout === tryoutDecisionConfirm.regId} onClick={() => void confirmTryoutDecision()}>
                Confirm
              </Button>
            </div>
          </div>
