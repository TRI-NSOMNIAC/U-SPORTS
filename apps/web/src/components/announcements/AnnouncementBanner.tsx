...
function announcementHeading(a: Announcement): string {
  if (a.type === 'emergency') return '⚠ EMERGENCY'
  if (a.type === 'reschedule') return '📅 RESCHEDULED'
  return a.title
}
...
function HeroTickerCopy({ a }: { a: Announcement }) {
  ...
  return (
    ...
    >
      {isCritical ? (
        <AlertTriangle className="w-4 h-4 text-[#FF3355] shrink-0" aria-hidden />
      ) : (
        <Clock className="w-4 h-4 text-[#FFB800] shrink-0" aria-hidden />
      )}
      <span
        className={cn(
          'text-sm font-bold tracking-widest uppercase',
          isCritical ? 'text-[#FF3355]' : 'text-[#FFB800]'
        )}
      >
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <span
              className={cn(
                'font-semibold mr-2',
                a.urgency === 'critical' ? 'text-[#FF3355]' : 'text-[#FFB800]'
              )}
            >
              {announcementHeading(a)}
            </span>
