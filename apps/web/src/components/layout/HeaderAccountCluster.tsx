...
function settingsPathForRole(role: string | undefined): string {
  if (role === 'super_admin') return '/super-admin/preferences'
...
export default function HeaderAccountCluster({ navVariant = 'app' }: Props) {
  ...
  const usesNotificationPopover = scopedProfile?.role === 'athlete' || scopedProfile?.role === 'student'
  ...
  const getNotifRoute = () => {
    if (scopedProfile?.role === 'organizer' || scopedProfile?.role === 'super_admin') return '/organizer/announcements'
    return '/guest'
  }
