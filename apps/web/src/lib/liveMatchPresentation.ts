...
function clampPeriod(p: number) {
  return Math.min(Math.max(p, 1), 5)
}

/** Total basketball points: quarters + OT, or `total` column when period rows were not stored. */
function basketballScoreSheetTotal(sc: Partial<MatchScore> | undefined): number {
  const fromPeriods =
    nz(sc?.q1) + nz(sc?.q2) + nz(sc?.q3) + nz(sc?.q4) + nz(sc?.ot)
...
export function liveScorePresentation(
  ...
  if (sport === 'table-tennis') {
    const keys = ['game1', 'game2', 'game3', 'game4', 'game5'] as const
    const k = keys[p - 1]
    return {
      left: Number(sa?.[k] ?? 0),
      right: Number(sb?.[k] ?? 0),
...
export function periodScoreBreakdown(
  ...
  if (sport === 'basketball') {
    ...
    const otL = nz(sa?.ot)
    const otR = nz(sb?.ot)
    if (sa?.ot != null || sb?.ot != null || otL > 0 || otR > 0) {
      rows.push({ label: 'OT', left: otL, right: otR })
    }

    const quarterSum =
      nz(sa?.q1) + nz(sa?.q2) + nz(sa?.q3) + nz(sa?.q4) + nz(sa?.ot) +
      nz(sb?.q1) + nz(sb?.q2) + nz(sb?.q3) + nz(sb?.q4) + nz(sb?.ot)
